package spring.board.post.service;
...
import spring.board.post.dto.PostDto;
...
public class PostService {
    ...
    public void deletePost(Long id, String password, Long loginMemberId){
        ...
        if(loginMemberId!=null){ //1. 관리자일 경우 무조건 삭제
            loginMember = memberRepository.findById(loginMemberId).orElseThrow(() -> new NotFoundException(ErrorCode.MEMBER_NOT_FOUND));
        ...
        if(post.getMember()!=null){ //2. 게시물 작성자가 회원일 경우
            ...
            if(password ==null || !passwordEncoder.matches(password,post.getGuestPassword())){
                throw new ForbiddenException(ErrorCode.INVALID_GUEST_PASSWORD);
            }
    ...
    public Post getPostAndIncreaseViewCount(Long id){
        ...
        if(updatedCount ==0) throw new NotFoundException(ErrorCode.POST_NOT_FOUND);

        return postRepository.findPostDetailById(id).orElseThrow(() -> new NotFoundException(ErrorCode.POST_NOT_FOUND));
    }
    ...
    public List<Post> getAllPost(){ //원래 메인 화면용이었는데 페이징방식으로 바꾸면서 사용안하는듯?
        return postRepository.findAll(Sort.by(Sort.Direction.DESC, "id"));
    }

    public void deleteAllPost(){ //개발할때 임시로 만들어뒀던건데 얘도 사용안하는듯
    ...
    private void validatePostDto(PostDto postDto, String sanitizedContent, boolean hasExistingImage,
        ...
        if (!StringUtils.hasText(postDto.getTitle())) {
            throw new InvalidRequestException(ErrorCode.POST_TITLE_REQUIRED);
        }
        if (!hasTextContent(sanitizedContent) && !hasUploadedImage(postDto) && !hasExistingImage) {
            throw new InvalidRequestException(ErrorCode.POST_CONTENT_OR_IMAGE_REQUIRED);
        }
        if (requirePoster && !StringUtils.hasText(postDto.getPoster())) {
            throw new InvalidRequestException(ErrorCode.GUEST_POSTER_REQUIRED);
        }
        if (requireGuestPassword && !StringUtils.hasText(postDto.getGuestPassword())) {
            throw new InvalidRequestException(ErrorCode.GUEST_PASSWORD_REQUIRED);
        }
    }
    ...
    public Long uploadPost(Long loginMemberId, PostDto postDto, String draftToken) throws IOException {
        ...
        if (loginMemberId !=null){ //작성자가 회원일때
            Member member = memberRepository.findById(loginMemberId).orElseThrow(() -> new NotFoundException(ErrorCode.MEMBER_NOT_FOUND));
    ...
    public Page<Post> searchPosts(String type, String keyword, int page){
        if(keyword==null){
            throw new InvalidRequestException(ErrorCode.SEARCH_KEYWORD_REQUIRED);
        }

        keyword = keyword.trim();

        if(keyword.length() < 2){
            throw new InvalidRequestException(ErrorCode.SEARCH_KEYWORD_TOO_SHORT);
        }
